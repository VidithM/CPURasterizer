...
std::vector<size_t> texWidths, texHeights;
...
std::string mainName = "Assignment3 - Vidith Madhu";
...
void handleColorMode(){
	for(int i = 0; i < triangleVector.size(); i++){
		if(colorMode == 0){
			...
			glm::vec3 assignCol(randX, randY, randZ);
			for(int j = 0; j < 3; j++){
				triangleVector[i].setColor(j, assignCol);
			}
		} else if (colorMode == 1){
			for(int j = 0; j < 3; j++){
				float randX = ((float)rand() / RAND_MAX);
			...
			for(int j = 0; j < 3; j++){
				float zVal = triangleVector[i].getVert(j).z;
				float dif = maxZ - minZ;
				zVal = (zVal - minZ) / dif;
				glm::vec3 assignCol(0.0f, 0.0f, zVal);
				triangleVector[i].setColor(j, assignCol);
			}
		}
	}
...
{	
	...
	{
		for (int i = 0; i < triangleVector.size(); i++){
			int mode = isTextured ? textureMode : -1;
			triangleVector[i].RenderCPU(modelViewMatrix, projectionMatrix, WINDOW_WIDTH, WINDOW_HEIGHT, &color[0][0][0], texture, texHeights, texWidths, mode, &depth[0][0]);
		}
...
void CreateTriangleVector(std::vector<glm::vec3> &vertices, std::vector<glm::vec2>& texCoords)
{
	for (int i = 0; i < vertices.size() / 3; i++)
	{
		Triangle myTriangle;
		for(int j = 0; j < 3; j++){
			minZ = std::min(minZ, vertices[i * 3 + j].z);
			maxZ = std::max(maxZ, vertices[i * 3 + j].z);
		}
		if (texCoords.empty())
...
void LoadTexture(char* name)
{
	...
	if (!image)
		...
		texWidths.push_back(texWidth);
		texHeights.push_back(texHeight);
	...
	for (int i = 0; i < numLevels; i++)
	{
		...
		//texWidths.push_back(curWidth);
		//texHeights.push_back(curHeight);
...
void Init()
{
	...
	std::cout << "Enter obj path: " << std::endl;
	std::cin >> modelPath;
	std::vector<glm::vec3> vertices;
	std::vector<glm::vec2> texCoords;
	LoadModel(modelPath.c_str(), vertices, texCoords);
